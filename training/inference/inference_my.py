...
def sliding_window_inference_my(
    ...
    roi_size: Sequence[int] | int,
    sw_batch_size: int,
    img_encoder, prompt_encoder, mask_decoder,
    overlap: Sequence[float] | float = 0.25,
    mode: BlendMode | str = BlendMode.CONSTANT,
    sigma_scale: Sequence[float] | float = 0.125,
    padding_mode: PytorchPadMode | str = PytorchPadMode.CONSTANT,
    ...
    **kwargs: Any,
    ...
    for slice_g in tqdm(windows_range) if progress else windows_range:
        ...
        if sw_batch_size > 1:
            ...
            win_data = inputs[unravel_slice[0]].to(sw_device)
        # seg_prob_out = predictor(win_data, *args, **kwargs)  # batched patch
        seg_prob_out =  give_prediction(win_data, img_encoder, prompt_encoder, mask_decoder, patch_size=128)
...
def give_prediction(inputs, img_encoder, prompt_encoder, mask_decoder, patch_size=128):
    ...
    img_resize = F.interpolate(inputs.permute(0, 1, 3, 4, 2), scale_factor=64/patch_size,
